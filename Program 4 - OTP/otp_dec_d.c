...
* from otp_dec client.  The textfile will be
* decrypt against the key.  Once the textfile
* has been decrypted, the data will be sent
* back to the client.
	...
	while (1)
	{
		...
		if (spawnPid == 0)
		{
			...
			if (verifyConnection(establishedConnectionFD) == true)
			{
				...
				//Decrypt the text file with key
				clearBuffer(decryp);
				decrypt(fileBuffer, keyBuffer, decryp);

				//Send decrypted data to client
				sendClient(establishedConnectionFD, decryp);
...
* the server is talking to otp_dec client
* and not otp_enc.
******************************************/
bool verifyConnection(int connectionFD)
{
	...
	if (strcmp(check, "dec") == 0)						//If code is valid
	{
		char respond[2] = "Y";							
...
/******************************************
*				decrypt
* This function is used to decrypt the
* textfile data that was sent by the client.
* This will use the key to calculate the 
* original message data.
******************************************/
void decrypt(char file[], char key[], char decryp[])
{
	int i;
	int intFile, intKey, intDecryp;
		...
		{	
			...
			intDecryp = intFile - intKey;				//Take the difference of text and file
			if (intDecryp < 0)							//Check if less then zero
				intDecryp += 27;						//Add 27 (modulus used)

			intDecryp += 65;							//Bring back to capital letters

			if (intDecryp == 91 || intDecryp == 64)		//If out of range then it is a space
				intDecryp = 32;							//Set to space

			decryp[i] = intDecryp;						//Place number into char array
		}
}
